...
async function handleGetItem(
  ...
  try {
    const item = await db.galeria.findUnique({
      where: { id },
      include: {
        autor: {
          select: {
...
async function handleUpdateItem(
  ...
  try {
    ...
    const itemActualizado = await db.galeria.update({
      where: { id },
      data: {
        tipo: tipo || item.tipo,
    ...
    await logAudit(req.user!.id, 'ACTUALIZAR', 'GALERIA', {
      galeriaId: id,
    });

    return sendSuccess(res, itemActualizado, 200, 'Item actualizado exitosamente');
  } catch (error) {
    return handleError(res, error);
  }
}
...
async function handleDeleteItem(
  ...
  try {
    ...
    await db.galeria.delete({
      where: { id },
    });

    await logAudit(req.user!.id, 'ELIMINAR', 'GALERIA', {
      galeriaId: id,
    });

    return sendSuccess(res, { id }, 200, 'Item eliminado exitosamente');
  } catch (error) {
    return handleError(res, error);
  }
}
